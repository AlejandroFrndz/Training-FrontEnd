        ...
        <div className="container">
            <div className="row justify-content-center">
                {characters.length > 0 ? characters.map((character,index) => {
                    return(
                        <div key={character.id} className={`col-4 me-2 mb-2 pb-3 ${styles.charWrapper} ${styles.overrideWidth}`}>
                            <Link to={`/characters/${character.id}`} className="text-decoration-none text-reset">
                                <CharacterPreview name={character.name} src={character.image} status={character.status}/>
                            </Link>
                        </div>
                    );
